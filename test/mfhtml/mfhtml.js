...
describe('MFHTML runtime', () => {
    ...
    it("should return meta of app on getMeta with keys ['appName', 'extending', 'alias', 'type', 'uses']", () => {
        mfhtml.register(mock.App);
        expect(mfhtml.getMeta('SampleApp')).to.have.all.keys(['appName', 'extending', 'alias', 'type', 'uses']);
        expect(mfhtml.getMeta('SampleApp')).to.not.have.all.keys(['bundle', 'overrides', 'publics', 'content', 'raw']);
    });
    ...
    it('should should throw error on getMissingDependencies for unregistered app', () => {
        expect(() => mfhtml.getMissingDependencies('SomeNonRegisteredApp')).to.throw(
            'SomeNonRegisteredApp is not registered!'
        );
    });
});
