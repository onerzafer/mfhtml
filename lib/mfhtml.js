...
class MFHTML {
    ...
    get(appName) {
        ...
        if (foundAppManifest.extends) {
            const parentDom = utils.parse(this.get(foundAppManifest.extends));
            appDom = Extend(parentDom, foundAppManifest);
        }
        if (foundAppManifest.uses && foundAppManifest.uses.length) {
            appDom = Import(
                appDom,
                foundAppManifest.uses.reduce(
                    (importedAppManifest, importedAppName) => ({
                        ...importedAppManifest,
