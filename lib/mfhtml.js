...
class MFHTML {
  ...
  get(appName, baseUrl = null) {
    ...
    let appDom = UrlFix(
      utils.parse(foundAppManifest.raw),
      baseUrl || foundAppManifest.baseUrl
    );
  ...
  getDependencies(appName) {
    ...
    return dependencies.reduce((cumulative, current) => [...cumulative, ...this.getDependencies(current)], []);
  }
  ...
  getMissingDependencies(appName) {
    ...
    const missingDeps = this.getDependencies(appName).filter(
      dependency => !appNames.includes(dependency)
    );
    return missingDeps.reduce((cumulative, current) => [...cumulative, ...this.getMissingDependencies(current)], []);
  }
}
