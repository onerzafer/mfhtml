    ...
    if (foundAppManifest.extending) {
      if (this.getAppNames().includes(foundAppManifest.extending)) {
        ...
        foundAppManifest.bundle = foundAppManifest.bundle.map(bundle => {
          const bundlePathsFixed = {
            ...bundle,
          };
          if (bundle.attributes && bundle.attributes.href) {
            bundlePathsFixed.attributes.href = utils.combineUrl(
              appBaseUrl,
              bundle.attributes.href
            );
          }
          if (bundle.attributes && bundle.attributes.src) {
            bundlePathsFixed.attributes.src = utils.combineUrl(
              appBaseUrl,
              bundle.attributes.src
            );
          }
          return bundlePathsFixed;
        });
